...
int main(int argc, char *argv[]) {
  if (argc == 1) {
    ...
    } else if ((count_flags > 0) && (argc < 4) && (!select.e || !select.e))
      print_msg();
    else {
...
void print_msg() { fprintf(stderr, "grep: invalid max count\n"); }

int get_flags(int argc, char *argv[], grep_flags *select, int *count_flags,
              char *templates) {
  int iserror = 0, isflag = 0, count_e = 0;
  while ((isflag = getopt_long(argc, argv, "e:ivclnhsf:o?", NULL, NULL)) !=
         -1) {
    switch (isflag) {
      case 'e':
        select->e = 1;
...
void comp_reg(char *argv[], char *templates, FILE *file, grep_flags *select,
  ...
  if (select->c) {
    if (count_files > 1 && !select->h && !select->l)
